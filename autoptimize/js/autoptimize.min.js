...
mashiro_global.variables=new function(){this.has_bot_ui=false;this.isNight=false;this.skinSecter=false;}
mashiro_global.ini=new function(){this.normalize=function(){lazyload();social_share();mashiro_global.post_list_show_animation.ini();copy_code_block();if(window.is_app){try{setTimeout(function(){mashiro_option.app_update(true);},10000);}catch(e){}}
if($("div").hasClass("poem-wrap")){get_poem('#poem','#info')}
$(function(){function waveloop1(){$("#banner_wave_1").css({"left":"-236px"}).animate({"left":"-1233px"},25000,'linear',waveloop1);}
function waveloop2(){$("#banner_wave_2").css({"left":"0px"}).animate({"left":"-1009px"},60000,'linear',waveloop2);}
if(screen&&screen.width>860){waveloop1();waveloop2();}
if(navigator.userAgent.indexOf('AppleWebKit')!=-1){$("body").addClass("isWebKit");}});hearthstone_deck_iframe();coverVideoIni();}
this.pjax=function(){pjaxInit();social_share();mashiro_global.post_list_show_animation.ini();copy_code_block();if($("div").hasClass("poem-wrap")){get_poem('#poem','#info')}
hearthstone_deck_iframe();coverVideoIni();mashiro_global.variables.pageTitile=document.title;}}
mashiro_global.lib=new function(){this.removeClass=function(ele,className){var el=document.getElementById(ele);if(el.classList)
el.classList.remove(className);else
el.className=el.className.replace(new RegExp('(^|\\b)'+className.split(' ').join('|')+'(\\b|$)','gi'),' ');}
this.addClass=function(ele,className){var el=document.getElementById(ele);if(el.classList)
el.classList.add(className);else
el.className+=' '+className;}
this.hasClass=function(ele,className){var el=document.getElementById(ele);if(el.classList)
var e=el.classList.contains(className);else
var e=new RegExp('(^| )'+className+'( |$)','gi').test(el.className);return e;}
this.toggleClass=function(ele,className){var el=document.getElementById(ele);if(el.classList){el.classList.toggle(className);}else{var classes=el.className.split(' ');var existingIndex=classes.indexOf(className);if(existingIndex>=0)
classes.splice(existingIndex,1);else
classes.push(className);el.className=classes.join(' ');}}
this.saveFile=function(url,file_name){var xhr=new XMLHttpRequest();xhr.responseType='blob';xhr.onload=function(){var a=document.createElement('a');a.href=window.URL.createObjectURL(xhr.response);a.download=file_name;a.style.display='none';document.body.appendChild(a);a.click();};xhr.open('GET',url);xhr.send();}}
...
function jumpTo(url){return mashiro_global.lib.pjax_to_url(url,'#page');}
function injectStyles(rule){var div=$("<div />",{html:'&shy;<style>'+rule+'</style>'}).appendTo("body");}
$('body').on('click','.comment-reply-link',function(e){addComment.moveForm("comment-"+$(this).attr('data-commentid'),$(this).attr('data-commentid'),"respond",$(this).attr('data-postid'));return false;});function tagHack(){var link=document.querySelector("link[rel*='icon']");mashiro_global.variables.title=document.title;window.onblur=function(){document.title=unescape('\u0028\u3064\u0020\u30a7\u0020\u2282\u0029\u6211\u85cf\u597d\u4e86\u54e6\u007e');link.href='https://static.2heng.xin/google.ico';};window.onfocus=function(){document.title=unescape('\u0028\u002a\u309c\u30ed\u309c\u0029\u30ce\u88ab\u53d1\u73b0\u5566\u007e');link.href='https://static.2heng.xin/favicon.ico';setTimeout(function(){document.title=mashiro_global.variables.title;},1000);}}
function imgError(ele,type){switch(type){case 1:if(ele.src.includes("https://cn.gravatar.com/avatar")){ele.src=ele.src.replace("https://cn.gravatar.com/avatar/","https://cdn.v2ex.com/gravatar/");}else{ele.src='https://view.moezx.cc/images/2017/12/30/Transparent_Akkarin.th.jpg';}
break;case 2:ele.src='https://gravatar.shino.cc/avatar/?s=80&d=mm&r=g';break;case 3:if(ele.src.includes("https://static.2heng.xin/")){ele.src=ele.src.replace("https://static.2heng.xin/wp-content/uploads/","https://cdn.2heng.xin/");}else{ele.src='https://view.moezx.cc/images/2018/05/13/image-404.png';}
break;default:ele.src='https://view.moezx.cc/images/2018/05/13/image-404.png';}}
mashiro_global.post_list_show_animation=new function(){this.ini=function(ajax){$("article.post-list-thumb").each(function(i){if(ajax){var window_height=$(window).height();}else{if($(".headertop").hasClass("headertop-bar")){var window_height=0;}else{if(mashiro_option.land_at_home){var window_height=$(window).height()-300;}else{var window_height=$(window).height();}}}
var article_height=$("article.post-list-thumb").eq(i).offset().top;if($(window).height()+$(window).scrollTop()>=article_height)
$("article.post-list-thumb").eq(i).addClass('post-list-show');$(window).scroll(function(){var scrolltop=$(window).scrollTop();if(scrolltop+window_height>=article_height&&scrolltop)
...
var ele_name=$('pre:eq('+i+')')[0].children[0].className;var lang=ele_name.substr(0,ele_name.indexOf(" ")).replace('language-','');if(lang.toLowerCase()=="hljs")var lang="text";if(lang.toLowerCase()=="js")var lang="javascript";if(lang.toLowerCase()=="md")var lang="markdown";if(lang.toLowerCase()=="py")var lang="python";$('pre:eq('+i+')').addClass('highlight-wrap');for(var t in attributes){$('pre:eq('+i+')').attr(t,attributes[t]);}
...
function click_to_view_image(){$(".comment_inline_img").click(function(){var temp_url=$(this).attr('src');window.open(temp_url);});}
...
if(mashiro_global.variables.isNight){$(".changeSkin-gear, .toc").css("background","rgba(255,255,255,0.8)");}else{$(".changeSkin-gear, .toc").css("background","none");}
$(document).ready(function(){function changeBG(tagid,url){$(".skin-menu "+tagid).click(function(){mashiro_global.variables.skinSecter=true;mashiro_global.variables.isNight=false;$("#night-mode-cover").css("visibility","hidden");$("body").css("background-image","url("+url+")");$(".blank").css("background-color","rgba(255,255,255,1)");$(".pattern-center").removeClass('pattern-center').addClass('pattern-center-sakura');$(".headertop-bar").removeClass('headertop-bar').addClass('headertop-bar-sakura');$('#banner_wave_1').addClass('banner_wave_hide_fit_skin');$('#banner_wave_2').addClass('banner_wave_hide_fit_skin');closeSkinMenu();setCookie("bgImgSetting",url,30);});}
function changeBGnoTrans(tagid,url){$(".skin-menu "+tagid).click(function(){mashiro_global.variables.skinSecter=true;mashiro_global.variables.isNight=true;$("#night-mode-cover").css("visibility","hidden");$("body").css("background-image","url("+url+")");$(".blank").css("background-color","rgba(255,255,255,1)");$(".pattern-center").removeClass('pattern-center').addClass('pattern-center-sakura');$(".headertop-bar").removeClass('headertop-bar').addClass('headertop-bar-sakura');$('#banner_wave_1').addClass('banner_wave_hide_fit_skin');$('#banner_wave_2').addClass('banner_wave_hide_fit_skin');closeSkinMenu();setCookie("bgImgSetting",url,30);});}
...
$(".changeSkin-gear").click(function(){$(".skin-menu").toggleClass('show');if(mashiro_global.variables.isNight){$(".changeSkin").css("background","rgba(255,255,255,0.8)");}else{$(".changeSkin").css("background","none");}})
$(".skin-menu #close-skinMenu").click(function(){closeSkinMenu();});add_upload_tips();});function hide_live2d(){if(getCookie("live2d")=="Hide"){setTimeout(function(){$(".prpr").css("visibility","hidden");if(document.body.clientWidth>860){$(".hide-live2d").css("bottom","66px");$(".save-live2d, .switch-live2d").addClass("hide-live2d-tool");}
$(".hide-live2d .keys").html("Show");setCookie("live2d","Show",7);},00);}else{setTimeout(function(){$(".prpr").css("visibility","visible");if(document.body.clientWidth>860){$(".hide-live2d").css("bottom","156px");$(".save-live2d, .switch-live2d").removeClass("hide-live2d-tool");}
$(".hide-live2d .keys").html("Hide");setCookie("live2d","Hide",7);},00);}}
mashiro_global.ini.live2d=function(){if(!getCookie("live2d"))setCookie("live2d","Hide",7);if(getCookie("live2d")=="Show"){setCookie("live2d","Hide",7);hide_live2d();}}
mashiro_global.ini.live2d();var bgn=1;function nextBG(){$(".centerbg").css("background-image","url(https://api.2heng.xin/cover/?"+bgn+")");bgn=bgn+1;}
function preBG(){bgn=bgn-1;$(".centerbg").css("background-image","url(https://api.2heng.xin/cover/?"+bgn+")");}
...
function reload_show_date_time(){BirthDay=new Date("06/02/2017 18:00:00");today=new Date();timeold=(today.getTime()-BirthDay.getTime());sectimeold=timeold/1000
secondsold=Math.floor(sectimeold);msPerDay=24*60*60*1000
e_daysold=timeold/msPerDay
daysold=Math.floor(e_daysold);monitorday.innerHTML=daysold;}
...
var al_expand_collapse_click=0;$('#al_expand_collapse').click(function(){if(al_expand_collapse_click==0){$al_post_list.show();al_expand_collapse_click++;}else if(al_expand_collapse_click==1){$al_post_list.hide();al_expand_collapse_click--;}});})();}}
timeSeriesReload();function coverVideo(){var video=document.getElementById("coverVideo");var btn=document.getElementById("coverVideo-btn");if(video.paused){video.play();try{btn.innerHTML='<i class="fa fa-pause" aria-hidden="true"></i>';}catch(e){};}else{video.pause();try{btn.innerHTML='<i class="fa fa-play" aria-hidden="true"></i>';}catch(e){};}}
function killCoverVideo(){var video=document.getElementById("coverVideo");var btn=document.getElementById("coverVideo-btn");if(video.paused){}else{video.pause();try{btn.innerHTML='<i class="fa fa-play" aria-hidden="true"></i>';}catch(e){};}}
function coverVideoIni(){if($('video').hasClass('hls')){var video=document.getElementById('coverVideo');var video_src=$('#coverVideo').attr('data-src');if(Hls.isSupported()){var hls=new Hls();hls.loadSource(video_src);hls.attachMedia(video);hls.on(Hls.Events.MANIFEST_PARSED,function(){video.play();});}else if(video.canPlayType('application/vnd.apple.mpegurl')){video.src=video_src;video.addEventListener('loadedmetadata',function(){video.play();});}}}
...
$('.iconflat').css('width','50px').css('height','50px');$('.openNav').css('height','50px');$("#bg-next").click(function(){nextBG();});$("#bg-pre").click(function(){preBG();});smileBoxToggle();timeSeriesReload();add_copyright();tableOfContentScroll(flag=true);console.log($("#myscript").text());}
...
if($("div").hasClass("popcontainer")){loadBotui();}
$("bot-ui").click(function(){loadBotui();});function add_copyright(){document.body.addEventListener("copy",function(e){if(!mashiro_global.is_user_logged_in&&window.getSelection().toString().length>30){setClipboardText(e);}
...
$('input#url').on('blur',function(){var URL_Address=$('input#url').val();$('input#url').val(URL_Address);setCookie('user_url',URL_Address,30);});if(getCookie('user_author')){$('input#author').val(getCookie('user_author'));}
$('input#author').on('blur',function(){var user_name=$('input#author').val();$('input#author').val(user_name);setCookie('user_author',user_name,30);});}
...
function loadBotui(){if($('div').hasClass('popcontainer')){if(mashiro_global.variables.has_bot_ui){bot_ui_ini();}else{$.getScript('https://cdn.jsdelivr.net/gh/moezx/cdn@latest/js/botui/botui.js',function(){bot_ui_ini();mashiro_global.variables.has_bot_ui=true;});}}}
function mail_me(){var mail="mailto:"+mashiro_option.email_name+"@"+mashiro_option.email_domain;window.open(mail);}
function hearthstone_deck_iframe(){if($("iframe").hasClass("hearthstone-deck")){$(".hearthstone-deck").each(function(){$(this).attr('height',$(this).width()*5/9+'px');});$(".hearthstone-deck-container").each(function(){var deck_container_height_fix=$(this).width()*5/9+14;$(this).css("height",deck_container_height_fix+"px");});}}
...
mashiro_global.ini.normalize();if(window.location.hostname=="lovelive.2heng.xin"){Poi.ajaxurl=Poi.ajaxurl.replace('//2heng.xin','');}
var home=location.href,s=$('#bgvideo')[0],Siren={MN:function(){$('.iconflat').on('click',function(){if($("#main-container").hasClass("open")){$('.iconflat').css('width','50px').css('height','50px');$('.openNav').css('height','50px');}else{$('.iconflat').css('width','100%').css('height','100%');$('.openNav').css('height','100%');}
$('body').toggleClass('navOpen');$('#main-container,#mo-nav,.openNav').toggleClass('open');});},MNH:function(){if($('body').hasClass('navOpen')){$('body').toggleClass('navOpen');$('#main-container,#mo-nav,.openNav').toggleClass('open');}},splay:function(){$('#video-btn').addClass('video-pause').removeClass('video-play').show();$('.video-stu').css({"bottom":"-100px"});$('.focusinfo').css({"top":"-999px"});$('#banner_wave_1').addClass('banner_wave_hide');$('#banner_wave_2').addClass('banner_wave_hide');for(var i=0;i<ap.length;i++){try{ap[i].destroy()}catch(e){}}
...
element.focus();}},false);}
loadCSS(mashiro_option.jsdelivr_css_src);loadCSS("https://at.alicdn.com/t/font_679578_dishi1yoavm.css");loadCSS("https://cdn.jsdelivr.net/gh/moezx/cdn@3.5.4/fonts/Moe-Mashiro/stylesheet.css");loadCSS("https://fonts.googleapis.com/css?family=Noto+SerifMerriweather|Merriweather+Sans|Source+Code+Pro|Ubuntu:400,700|Noto+Serif+SC");loadCSS("https://cdn.jsdelivr.net/gh/moezx/cdn@3.3.9/css/sharejs.css");;
function checkPIOCookie(){var donotneed=getCookie("dontwantlive2d");if(donotneed!=""){if(donotneed=="yes"){$(".hide-live2d").css("visibility","hidden");$(".prpr").css("visibility","hidden");console.log("If you want to see live2d please clean cookie!");}else{loadlive2d("live2d","https://cdn.jsdelivr.net/gh/moezx/live2d@v1.0/live2d/Pio/appv4.json");}}else{loadlive2d("live2d","https://cdn.jsdelivr.net/gh/moezx/live2d@v1.0/live2d/Pio/appv4.json");}}
var Live2D_img_path="";function pio(){var Live2D_file_domain='https://cdn.jsdelivr.net/gh/moezx/cdn@3.4.9/img/Sakura/images/lagrange/';var live2d_file_id=Math.ceil(Math.random()*92);var Live2D_file_name='pio ('+live2d_file_id+').png';Live2D_img_path=Live2D_file_domain+MD5(Live2D_file_name);loadlive2d("live2d","https://cdn.jsdelivr.net/gh/moezx/live2d@v1.0/live2d/Pio/appv4.json");}
function tia(){Live2D_img_path="https://calculus.shino.cc/eulerian/";loadlive2d("live2d","https://cdn.jsdelivr.net/gh/moezx/live2d@v1.0/live2d/Tia/appv2.json");}
function switch_pio(){if(isIE||isEdge){addComment.createButterbar("此功能不支持您的浏览器<br>Feature cannot work on your browser");return false;}else if(isChrome){pio();}else if(isSafari){addComment.createButterbar("此功能不支持您的浏览器<br>Feature cannot work on your browser");return false;}else{pio();}}
function save_pio(){window.Live2D.captureName='Screenshot-'+Date.now()+'.png';window.Live2D.captureFrame=true;addComment.createButterbar("保存成功！<br>Screenshot saved!",1000);}
if(!window.is_app){var userAgent=navigator.userAgent;console.log('userAgent = '+userAgent);console.log('window inner size: '+window.innerWidth+' x '+window.innerHeight);var isOpera=userAgent.indexOf("Opera")>-1;var isChrome=navigator.userAgent.toLowerCase().indexOf('chrome')>-1;var isIE=userAgent.indexOf("compatible")>-1&&userAgent.indexOf("MSIE")>-1&&!isOpera;var isEdge=userAgent.indexOf("Edge")>-1;var isSafari=userAgent.indexOf("Safari")>-1;if(isIE||isEdge){Live2D_img_path='https://2heng.xin/live2d/Pio/api/';loadlive2d("live2d","https://cdn.jsdelivr.net/gh/moezx/live2d@v1.0/live2d/Pio/model-default.json");}else if(isChrome){pio();}else if(isSafari){Live2D_img_path='https://2heng.xin/live2d/Pio/api/';loadlive2d("live2d","https://cdn.jsdelivr.net/gh/moezx/live2d@v1.0/live2d/Pio/model-default.json");}else{pio();}};
function render(template,context){var tokenReg=/(\\)?\{([^\{\}\\]+)(\\)?\}/g;return template.replace(tokenReg,function(word,slash1,token,slash2){if(slash1||slash2){return word.replace('\\','');}
...
String.prototype.render=function(context){return render(this,context);};var re=/x/;re.toString=function(){showMessage('哈哈，你打开了控制台，是想要看看我的秘密吗？',5000);return'';};$(document).on('copy',function(){showMessage('你都复制了些什么呀，转载要记得加上出处哦',5000);});$.ajax({cache:true,url:"https://cdn.jsdelivr.net/gh/moezx/live2d@v1.3/live2d/tips.json",dataType:"json",success:function(result){$.each(result.mouseover,function(index,tips){$(document).on("mouseover",tips.selector,function(){var text=tips.text;if(Array.isArray(tips.text))text=tips.text[Math.floor(Math.random()*tips.text.length+1)-1];text=text.render({text:$(this).text()});showMessage(text,3000);});});$.each(result.click,function(index,tips){$(document).on("click",tips.selector,function(){var text=tips.text;if(Array.isArray(tips.text))text=tips.text[Math.floor(Math.random()*tips.text.length+1)-1];text=text.render({text:$(this).text()});showMessage(text,5000);});});}});(function(){var text;if(document.referrer!==''){var referrer=document.createElement('a');referrer.href=document.referrer;text='Hello! 来自 <span style="color:#E06020;">'+referrer.hostname+'</span> 的朋友~';var domain=referrer.hostname.split('.')[1];if(domain=='baidu'){text='Hello! 从 百度 进来的朋友<br>欢迎阅读<span style="color:#E06020;';}else if(domain=='so'){text='Hello! 用 360搜索 找到我的朋友<br>欢迎阅读<span style="color:#E06020;';}else if(domain=='sogou'){text='Hello! 用 搜狗搜索 找到我的朋友<br>欢迎阅读<span style="color:#E06020;';}else if(domain=='bing'){text='Hello! 用 必应 找到我的朋友<br>欢迎阅读<span style="color:#E06020;';}else if(domain=='2heng'){text='只要微笑就可以了 ^_^';}else if(domain=='google'){text='Hello! 来自 Google 的朋友<br>欢迎阅读<span style="color:#E06020;">『'+document.title.split(' - ')[0]+'』</span>';}}else{if(window.location.href=='https://2heng.xin/'){var now=(new Date()).getHours();if(now>23||now<=5){text='你是夜猫子呀？这么晚还不睡觉，明天起得来嘛?';}else if(now>5&&now<=7){text='早上好！一日之计在于晨，美好的一天就要开始了';}else if(now>7&&now<=11){text='上午好！工作顺利嘛？不要久坐，多起来走动走动哦！';}else if(now>11&&now<=14){text='中午了，工作了一个上午，现在是午餐时间！';}else if(now>14&&now<=17){text='午后很容易犯困呢，幸福地睡个午觉吧？';}else if(now>17&&now<=19){text='傍晚了！窗外的夕阳很美丽呢~';}else if(now>19&&now<=21){text='晚上好，今天过得怎么样？';}else if(now>21&&now<=23){text='已经这么晚了呀，早点休息吧，晚安~';}else{text='嗨~ 快来逗我玩吧！';}}else if(window.location.href=='https://2heng.xin/about/'){text='Do you like me? ヾ(≧∇≦*)ゝ';}else{text='欢迎阅读<span style="color:#E06020;">『'+document.title.split(' - ')[0]+'』</span>';}}
showMessage(text,12000);})();window.setInterval(showHitokoto,30000);function showHitokoto(){$.getJSON('https://api.shino.cc/hitokoto/?encode=json',function(result){showMessage(result.hitokoto,16000);});}
function showMessage(text,timeout){if(Array.isArray(text))text=text[Math.floor(Math.random()*text.length+1)-1];$('.mashiro-tips').stop();$('.mashiro-tips').html(text).fadeTo(200,1);if(timeout===null)timeout=5000;hideMessage(timeout);}
function hideMessage(timeout){$('.mashiro-tips').stop().css('opacity',1);if(timeout===null)timeout=5000;$('.mashiro-tips').delay(timeout).fadeTo(200,0);}
$(document).ready(function(){setTimeout(function(){isFirstLoad=true;if(document.body.clientWidth>860){$(".changeSkin-gear").css("visibility","visible");}
$("p").remove(".head-copyright");},0)});if($("div").hasClass("aplayer")){reloadHermit();};
...
lrcTag=2;});var apSwitchTag=0;$(".aplayer.aplayer-fixed .aplayer-body").addClass("ap-hover");$(".aplayer-miniswitcher").click(function(){if(apSwitchTag==0){$(".aplayer.aplayer-fixed .aplayer-body").removeClass("ap-hover");apSwitchTag=1;}else{$(".aplayer.aplayer-fixed .aplayer-body").addClass("ap-hover");apSwitchTag=0;}});}
var b='https://api.i-meto.com/meting/api?server=:server&type=:type&id=:id&r=:r';'undefined'!=typeof meting_api&&(b=meting_api);for(var f=0;f<aplayers.length;f++)try{aplayers[f].destroy()}catch(a){console.log(a)}
