(function webpackUniversalModuleDefinition(b, a) {
	...
	return (function(c) {
		...
		(function() {
			var e = ["direction", "boxSizing", "width", "height", "overflowX", "overflowY", "borderTopWidth",
				...
				"fontSizeAdjust", "lineHeight", "fontFamily", "textAlign", "textTransform", "textIndent", "textDecoration",
...
$('body').on('click', '.comment-reply-link', function(e) {
	addComment.moveForm("comment-" + $(this).attr('data-commentid'), $(this).attr('data-commentid'), "respond", $(this).attr(
		'data-postid'));
	return false;
});
				...
				success: function(res) {
					...
					if (res.status == 200) {
						...
						$('#upload-img-show').append(
							...
							get_the_url + '" onclick="window.open(\'' + get_the_url + '\')" onerror="imgError(this)" />');
						lazyload();
						addComment.createButterbar("图片上传成功~<br>Uploaded successfully~");
						grin(get_the_url, type = 'Img');
					} else {
						addComment.createButterbar("上传失败！<br>Uploaded failed!<br> 文件名/Filename: " + f.name + "<br>code: " + res.status +
							"<br>" + res.message, 3000);
					}
				},
				error: function() {
...
function checkBgImgCookie() {
	...
	if (bgurl != "") {
		if (bgurl == "https://view.moezx.cc/images/2018/01/03/sakura.png" || bgurl ==
			"https://view.moezx.cc/images/2018/01/03/plaid2dbf8.jpg" || bgurl ==
			"https://view.moezx.cc/images/2018/01/10/star02.png" || bgurl ==
			"https://view.moezx.cc/images/2018/01/24/kyotoanimation.png" || bgurl ==
			"https://view.moezx.cc/images/2018/09/17/little-monster.png") {
...
$(document).ready(function() {
	...
	$(".skin-menu #dark-bg").click(function() {
		...
		$("body").css("background-image",
			"url(https://cdn.jsdelivr.net/gh/moezx/cdn@3.1.2/other-sites/api-index/images/me.png)");
...
function copy_code_block() {
	$('pre code').each(function(i, block) {
		...
		$(this).after('<a class="copy-code" href="javascript:" data-clipboard-target="#hljs-' + i +
...
function add_copyright() {
	...
	function setClipboardText(event) {
		...
		var htmlData = "# 商业转载请联系作者获得授权，非商业转载请注明出处。<br>" +
			...
			"# 协议(License)：署名-非商业性使用-相同方式共享 4.0 国际 (CC BY-NC-SA 4.0)<br>" + "# 作者(Author)：" + mashiro_option.author_name +
			"<br>" + "# 链接(URL)：" + window.location.href + "<br>" + "# 来源(Source)：" + mashiro_option.site_name + "<br><br>" +
...
function inlojv_js_getqqinfo() {
	...
	if (getCookie('user_avatar') && getCookie('user_email') && getCookie('is_user_qq') == 'no' && !getCookie(
			'user_qq_email')) {
		$('div.comment-user-avatar img').attr('src', getCookie('user_avatar'));
	...
	function c() {
		if (!e) {
			...
			for (c = 0; c < i.length; c++) {
				if (d = i[c], !d.getAttribute("data-secret")) f = Math.random().toString(36).substr(2, 10), d.src += "#?secret=" +
					f, d.setAttribute("data-secret", f);
						...
						if (!/[^a-zA-Z0-9]/.test(d.secret)) {
							...
							for (e = 0; e < j.length; e++)
								if (f = j[e], c.source === f.contentWindow) {
									...
									if ("link" === d.message)
										if (h = b.createElement("a"), i = b.createElement("a"), h.href = f.getAttribute("src"), i.href = d.value, i.host ===
											h.host)
											if (b.activeElement === f) a.top.location.href = d.value
								} else;
						}
			}, d) a.addEventListener("message", a.wp.receiveEmbedMessage, !1), b.addEventListener("DOMContentLoaded", c, !1), a.addEventListener(
			"load", c, !1)
}(window, document);
